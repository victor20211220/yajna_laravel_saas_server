...
class CoingatePaymentController extends Controller
{
    ...
    public function planPayWithCoingate(Request $request)
    {
        ...
        if($plan)
        {
            ...
            try{
                ...
                // $client = new Client(config('environment',$this->mode), config('auth_token',$this->coingate_auth_token), config('curlopt_ssl_verifypeer', FALSE));
                $post_params = array(
                    'order_id' => time(),
                    'price_amount' => $price,
            ...
            if($order['status_code'] === 200) {
                ...
                session(['coingate_data' => $response]);
    ...
    public function getPaymentStatus(Request $request,$plan)
    {
        ...
        $coingate_order_id = session('coingate_data');
        $price                 = $plan->price;
        if($plan)
        {
                ...
                $order->price          = $coingate_order_id['price_amount'];
